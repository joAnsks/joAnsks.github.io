...
- **Balls:** player steers the main ball (`r: 16`, starts at rest); hitting cushions spawns a mini child-ball at the cushion centre; mini-balls grow (`r: 5 → 14` at `+0.02/frame`) and bounce autonomously; full-size minis can also spawn children; max 12 balls total; main ball is always larger than fully-grown minis (r 16 > 14) so it stays visually distinct
...
- **Path nodes:** `3 + level` nodes distributed evenly along the four canvas wall edges; initially faint (alpha 0.12); only a **fully-grown mini-ball** (`r ≥ 14`, `isMain === false`) passing within 18 px activates the node (glow + burst + `+50` score); the main ball and still-growing minis cannot trigger nodes; all activated → level complete
